...
output.dir <- '/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1b_eQTL_Analysis_TElocal/5_Replication_of_eQTLs_in_YRI/Scan_for_eQTLs/'

# Define the YRI gene/TE INT expression file path
Gene_expression.YRI.path <- "/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1b_eQTL_Analysis_TElocal/2_Prepare_RNASeq/Processed_counts_eQTL/TElocal_counts_YRI_86_filtered_aggregate_split_VST_BatchesRemoved_lab_ancestry_PCs_sex_TE_copies_EBV_INT_All_Genes_TEs.txt"
...
# SCAN FOR EUR EQTL REPLICATION IN YRI (INTERGENIC DISTAL) ----------------------------
...
sig.eQTLs <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1b_eQTL_Analysis_TElocal/3_Scan_for_eQTLs/Significant_SNVs/L1_intergenic_distal_trans_eQTLs_SIGNIFICANT_EUR_2023-11-15.txt", header = T, row.names = NULL, stringsAsFactors = F, sep = '\t')
...
EUR_snps_in_YRI <- EUR_snps_in_YRI[order(EUR_snps_in_YRI$pvalue, decreasing = FALSE), ]
    ...
    write.table(EUR_snps_in_YRI, file = paste(output.dir, "L1_intergenic_distal_subfamily_trans_eQTLs_YRI_ALL", '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)

# Filter by BH FDR (NO SIGNIFICANT RESULTS; LOOSEN THE FILTER FOR PLOTTING PURPOSES)
EUR_snps_in_YRI <- EUR_snps_in_YRI[which(EUR_snps_in_YRI$FDR < 0.15), ]

# Save data
write.table(EUR_snps_in_YRI, file = paste(output.dir, "L1_intergenic_distal_subfamily_trans_eQTLs_YRI_FDR15", '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)
...
# SCAN FOR EUR EQTL REPLICATION IN YRI (INTRONIC) ----------------------------
...
# Load EUR significant snps
sig.eQTLs <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1b_eQTL_Analysis_TElocal/3_Scan_for_eQTLs/Significant_SNVs/L1_intronic_trans_eQTLs_SIGNIFICANT_EUR_2023-11-15.txt", header = T, row.names = NULL, stringsAsFactors = F, sep = '\t')
...
sig.eQTLs <- sig.eQTLs[EUR.shared_sig_snp_indices, ]
    ...
    # Keep EUR stats only for repeats expressed in YRI
...
EUR_snps_in_YRI <- EUR_snps_in_YRI[order(EUR_snps_in_YRI$pvalue, decreasing = FALSE), ]
    ...
    # Save unfiltered results
    write.table(EUR_snps_in_YRI, file = paste(output.dir, "L1_intronic_subfamily_trans_eQTLs_YRI_ALL", '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)

# Filter by BH FDR (NO SIGNIFICANT RESULTS; LOOSEN THE FILTER FOR PLOTTING PURPOSES)
EUR_snps_in_YRI <- EUR_snps_in_YRI[which(EUR_snps_in_YRI$FDR < 0.05), ]

  # Save data
  write.table(EUR_snps_in_YRI, file = paste(output.dir, "L1_intronic_subfamily_trans_eQTLs_YRI_FDR5", '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)
...
# SCAN FOR EUR EQTL REPLICATION IN YRI (GENES) ----------------------------

    
        
# Run EUR gene cis-eQTLs
  
# Load EUR significant cis-eQTLs LINKED TO A TE SNV (NOTE: ALL YRI L1 FILES MAY NEED TO BE COLLECTED IF WE WANT TO DO THIS FOR ALL L1 CATEGORIES)
sig.eQTLs <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1b_eQTL_Analysis_TElocal/4_Integrate_cis_trans_eQTLs/Annotated_and_filtered_eQTLs/EUR_L1_intergenic_distal_eQTLs_annotated_0_filters.txt", header = T, row.names = NULL, stringsAsFactors = F, sep = '\t')
...
EUR_snps_in_YRI <- EUR_snps_in_YRI[order(EUR_snps_in_YRI$pvalue, decreasing = FALSE), ]
    ...
    write.table(EUR_snps_in_YRI, file = paste(output.dir, "Gene_TElocal_cis_eQTLs_YRI_ALL_", Sys.Date(), '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)

# Filter by BH FDR
EUR_snps_in_YRI <- EUR_snps_in_YRI[which(EUR_snps_in_YRI$FDR < 0.05), ]

    # Save data
    write.table(EUR_snps_in_YRI, file = paste(output.dir, "Gene_TElocal_cis_eQTLs_YRI_FDR5_", Sys.Date(), '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)
...
# INTEGRATE CIS AND TRANS QTLs AND PLOT (INTERGENIC DISTAL) (NOTHING SIGNIFICANT; DONT UPDATE OR RUN) ----------------------------
