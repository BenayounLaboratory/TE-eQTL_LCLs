...
# Define output dir
output.dir <- '/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/8_Mediation_Analysis/Results/'
...
combined_cis_trans.EUR <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/6_Integrate_cis_trans_eQTLs/Annotated_and_filtered_eQTLs/EUR_L1_eQTLs_annotated_1_filters_geneTEcorrelation_2023-05-22.txt", header = T, row.names = NULL, stringsAsFactors = F, sep = '\t')
...
# Run eqtlmapt in basic mode 
    ...
    # Save data
    write.table(mediation.results.EUR, file = paste(output.dir, "Mediation_Analysis_EUR_L1_Candidates_gmap_with_eQTL_data_", Sys.Date(), '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)
    ...
    mediation.results.YRI.sig <- mediation.results.YRI[which(mediation.results.YRI$EMP_P_FDR < 0.05), ]
        ...
        # Save data
        write.table(mediation.results.EUR.sig, file = paste(output.dir, "Mediation_Analysis_EUR_L1_Candidates_gmap_with_eQTL_data_FDR5_", Sys.Date(), '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)
...
mediation.EUR <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/8_Mediation_Analysis/Results/Mediation_Analysis_EUR_L1_Candidates_gmap_with_eQTL_data_FDR5_2022-11-15.txt", header = T, row.names = NULL, stringsAsFactors = F, sep = '\t')
...
pdf(paste(output.dir, "Plot_Most_Significant_Proportion_Mediated_EUR_", Sys.Date(), ".pdf", sep=""), width = 6, height = 6)
par(mar=c(6,4,4,1)+.1)
    ...
    text(1.9, 0.45, mediation.EUR[2, 'EMP_P_FDR'], cex = 1)
    text(3.1, 0.5, mediation.EUR[3, 'EMP_P_FDR'], cex = 1)
    text(4.3, 0.45, mediation.EUR[4, 'EMP_P_FDR'], cex = 1)
    text(5.5, 0.5, mediation.EUR[5, 'EMP_P_FDR'], cex = 1)
    text(6.7, 0.45, mediation.EUR[6, 'EMP_P_FDR'], cex = 1)
    text(7.9, 0.5, mediation.EUR[7, 'EMP_P_FDR'], cex = 1)
    text(9.1, 0.45, mediation.EUR[8, 'EMP_P_FDR'], cex = 1)
...
mediation.YRI$Indirect <- mediation.YRI$beta.total - mediation.YRI$beta
...
rownames(mediation.YRI) <- 1:nrow(mediation.YRI)
    
# Specify row numbers to plot (corresponding to SNV-gene interactions in the paper figures)
rows_to_plot.EUR <- c(220, 254, 224, 69, 119, 118) #NOTE: IL16, HSD17B12 SNVs are not the SNVs at the peaks
...
data_to_plot.EUR.pos <- data_to_plot.EUR[, -c(3)]
data_to_plot.EUR.neg <- data_to_plot.EUR[, c(3, 7)]
...
pdf(paste(output.dir, "Barplot_Direct_Indirect_Total_Effects_EUR_Positive",".pdf", sep=""), width = 7, height = 7)
...
pdf(paste(output.dir, "Barplot_Direct_Indirect_Total_Effects_EUR_Negative",".pdf", sep=""), width = 7, height = 7)
par(mfrow=c(3,3)) # Added to produce images of similar shape across analyses
