...
# STRUCTURAL VARIANT POPULATION STRUCTURE USING PLINK 
...
pca.EUR <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/1_Prepare_Genotype_Data/Structural_Variant_Genotypes/Plink_Pruned_EUR/Insertions_Filtered_EUR_BOTH_SEXES_PCA.eigenvec", header = FALSE, row.names = 1, sep = "")
pca.YRI <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/1_Prepare_Genotype_Data/Structural_Variant_Genotypes/Plink_Pruned_YRI/Insertions_Filtered_YRI_BOTH_SEXES_PCA.eigenvec", header = FALSE, row.names = 1, sep = "")
...
eigenval.EUR <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/1_Prepare_Genotype_Data/Structural_Variant_Genotypes/Plink_Pruned_EUR/Insertions_Filtered_EUR_BOTH_SEXES_PCA.eigenval", header = FALSE)
eigenval.YRI <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/1_Prepare_Genotype_Data/Structural_Variant_Genotypes/Plink_Pruned_YRI/Insertions_Filtered_YRI_BOTH_SEXES_PCA.eigenval", header = FALSE)
...
# Plot pve
pdf(paste(dir.output, "Plot_Structural_Variant_PCA_percent_variance", ".pdf", sep=""))
par(mfrow=c(2,1))
    barplot(t(pve.EUR), names.arg = rownames(pve.EUR), xlab = "Principal component", ylab = 'Percent variance explained', main = 'EUR SV PCA', las = 2)
    barplot(t(pve.YRI), names.arg = rownames(pve.YRI), xlab = "Principal component", ylab = 'Percent variance explained', main = 'YRI SV PCA', las = 2)
dev.off()



# Plot PC vs Ancestry
pdf(paste(dir.output, "Plot_Structural_Variant_PCs_vs_Ancestry",".pdf", sep=""), width = 7, height = 7)
par(mfrow=c(2,2)) # Added to produce images of similar shape across analyses
    ...
    plot(pca.EUR$PC1,
        ...
        # add legend
        legend('bottomleft', legend = EUR.labels, col = EUR.colors, pch = EUR.pch, cex = 1, lty = 0, bty = 'n')
    ...
    plot(pca.YRI$PC1,
        ...
        # add a legend
        legend('bottomright', legend = YRI.labels, col = YRI.colors, pch = YRI.pch, cex = 1, lty = 0, bty = 'n')
...
write.table(pca.EUR.YRI, file = paste(dir.output, "COVARIATE_Structural_Variant_Population_Structure_PCA", ".txt", sep =""), sep = "\t" , row.names = T, col.names = NA, quote=F)
