...
# # Annotate Alu eQTLs (NOT USED IN THE FINAL ANALYSIS)
#   
# # Load Alu trans-eQTLs passing FDR thresholding
# Alu.eQTLs <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/5_Scan_for_eQTLs/Significant_SNVs/Alu_trans_eQTLs_SIGNIFICANT_EUR_2022-10-26.txt", header = T, row.names = NULL, stringsAsFactors = F, sep = '\t')
# 
# # Load Alu trans-eQTL clumping results
# Alu.clumps <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/5_Scan_for_eQTLs/Significant_SNVs/Alu_trans_eQTLs_SIGNIFICANT_EUR.clumped", header = TRUE, row.names = NULL, stringsAsFactors = F, sep = "")
# 
# # Load cis-eQTLs passing FDR thresholding
# cis.eQTLs <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/1_eQTL_Analysis/5_Scan_for_eQTLs/Significant_SNVs/Gene_cis_eQTLs_SIGNIFICANT_EUR_2023-05-22.txt", header = T, row.names = NULL, stringsAsFactors = F, sep = '\t')
# 
#     # Only keep cis-eQTL snps that are trans-eQTLs (use the snps column, a custom snp ID that takes into account duplicate RsIDs)
#     cis.eQTLs <- cis.eQTLs[which(cis.eQTLs$snps %in% Alu.eQTLs$snps), ]
#     
# # Link TEs and Genes using the shared snps
# combined_cis_trans <- link_cis_and_trans_eQTLs(cis.sig = cis.eQTLs, 
#                                                trans.sig = Alu.eQTLs, 
...
# 
#     # Save data
#     write.table(combined_cis_trans, file = paste(output.dir, "EUR_Alu_eQTLs_annotated_0_filters_", Sys.Date(), '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)
#     
# # Filter on linear regression FDR
# combined_cis_trans_1_filter <- combined_cis_trans[which(combined_cis_trans$Gene_TE_FDR < 0.05), ] 
# 
#     # Save data
#     write.table(combined_cis_trans_1_filter, file = paste(output.dir, "EUR_Alu_eQTLs_annotated_1_filters_geneTEcorrelation_", Sys.Date(), '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)
#     
# # filter by whether snps are independent (based on Plink R^2)
# combined_cis_trans_1_filter <- combined_cis_trans[which(combined_cis_trans$RsID %in% Alu.clumps$SNP), ] # Unique genes left are STARD5, IL16, HLA-DRB5, HLA-DQA2, HSD17B12   
# 
#     # Save data
#     write.table(combined_cis_trans_1_filter, file = paste(output.dir, "EUR_Alu_eQTLs_annotated_1_filters_LD_", Sys.Date(), '.txt', sep =""), sep = "\t" , row.names = F, col.names = T, quote = F)
