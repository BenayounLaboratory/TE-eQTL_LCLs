...
library(DOSE)
library(poolr) # To calculate meta pvalues, used for ranking gene sets
...
my.output.dir <- '/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/2_RNASeq_Analysis_Top_eQTLs/2_GSEA/Compare_Top_3_SNVs/'
...
overlap.Hallmark <- Overlap_three_GSEA_res(result.one = Hallmark.IL16@result,
                                         ...
                                         comparison_directionality = c('same', 'opposite'),
                                         number_to_plot = 10,
                                         dir.output = my.output.dir,
                                         gs.label = "Hallmark")

overlap.Reactome <- Overlap_three_GSEA_res(result.one = Reactome.IL16@result,
...
overlap.GOBP <- Overlap_three_GSEA_res(result.one = GOBP.IL16@result,
...
overlap.TFT <- Overlap_three_GSEA_res(result.one = TFT.IL16@result,
...
overlap.mirdb <- Overlap_three_GSEA_res(result.one = miRDB.IL16@result,
...
overlap.TE_families <- Overlap_three_GSEA_res(result.one = Family.EUR.IL16@result,
...
sink(file = paste(dir.session_info, "Session_Info_GSEA_Comparing_Top_3_SNVs.txt", sep =""))
sessionInfo()
sink()      
