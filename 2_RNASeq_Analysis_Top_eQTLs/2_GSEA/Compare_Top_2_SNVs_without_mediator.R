...
library(DOSE)
library(poolr) # for meta pvalue calculations





# COMPARE GSEA RESULTS (rs112581165 no candidate mediator vs rs72691418 no candidate mediator) 

# GSEA directory for output files
my.output.dir <- '/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/2_RNASeq_Analysis_Top_eQTLs/2_GSEA/Compare_Top_2_SNVs_without_mediator/'
...
# Define plot labels
my.label.one <- 'rs112581165'
my.label.two <- 'rs72691418'


# Find overlapping gene sets   
overlap.Hallmark <- Overlap_two_GSEA_res(result.one = Hallmark.SNV4@result,
                                         result.two = Hallmark.SNV5@result,
                                         label.one = my.label.one,
                                         label.two = my.label.two,
                                         comparison_directionality = 'opposite',
                                         number_to_plot = 10,
                                         dir.output = my.output.dir,
                                         gs.label = "Hallmark")

overlap.Reactome <- Overlap_two_GSEA_res(result.one = Reactome.SNV4@result,
                                         result.two = Reactome.SNV5@result,
                                         label.one = my.label.one,
                                         label.two = my.label.two,
                                         comparison_directionality = 'opposite',
                                         number_to_plot = 10,
...
overlap.GOBP <- Overlap_two_GSEA_res(result.one = GOBP.SNV4@result,
                                         result.two = GOBP.SNV5@result,
                                         label.one = my.label.one,
                                         label.two = my.label.two,
                                         comparison_directionality = 'opposite',
                                         number_to_plot = 10,
...
overlap.TFT <- Overlap_two_GSEA_res(result.one = TFT.SNV4@result,
                                       result.two = TFT.SNV5@result,
                                       label.one = my.label.one,
                                       label.two = my.label.two,
                                       comparison_directionality = 'opposite',
                                       number_to_plot = 10,
...
overlap.mirdb <- Overlap_two_GSEA_res(result.one = miRDB.SNV4@result,
                                       result.two = miRDB.SNV5@result,
                                       label.one = my.label.one,
                                       label.two = my.label.two,
                                       comparison_directionality = 'opposite',
                                       number_to_plot = 10,
...
overlap.TE_families <- Overlap_two_GSEA_res(result.one = Family.EUR.SNV4@result,
                                             result.two = Family.EUR.SNV5@result,
                                             label.one = my.label.one,
                                             label.two = my.label.two,
...
sink(file = paste(dir.session_info, "Session_Info_GSEA_Comparing_Top_2_SNVs_without_mediator.txt", sep =""))
sessionInfo()
sink()      
