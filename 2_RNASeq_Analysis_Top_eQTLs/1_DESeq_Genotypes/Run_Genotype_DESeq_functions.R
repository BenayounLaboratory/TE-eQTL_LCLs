...
Run_DESeq <- function(all_covariates, SNV_RsID, target_population, counts_df, my.output.dir, my.output.prefix){
  
  
  # This function runs DESeq comparing the genotypes of an SNV of interest in either EUR or YRI samples from the Geuvadis project
  ...
  if (target_population == 'YRI') {
      ...
      SampleInfo <- all_covariates[, c('sex', 'lab', 'PC1', 'PC2', 'SV_PC1', 'SV_PC2', 'SV_PC3', SNV_RsID)]
      
      # obtain the index for the last column
      SNV_column_index <- ncol(SampleInfo)
      
      # Update the last column name (the SNV being analyzed) to a generic name that can be called in the DESeq model
      colnames(SampleInfo)[SNV_column_index] <- 'SNV1'
      
      # Create DESeq2 object
      dds <- DESeqDataSetFromMatrix(countData = counts_df,
                                    colData = SampleInfo,
                                    design = ~ sex + lab + PC1 + PC2 + SV_PC1 + SV_PC2 + SV_PC3 + SNV1) 
      
  } else if (target_population == 'EUR') {
    
      # Define the final set of covariates to include in the DESeq model
      SampleInfo <- all_covariates[, c('sex', 'lab', 'ancestry', 'PC1', 'PC2', 'SV_PC1', 'SV_PC2', 'SV_PC3', SNV_RsID)]
      ...
      dds <- DESeqDataSetFromMatrix(countData = counts_df,
                                    colData = SampleInfo,
                                    design = ~ sex + lab + ancestry + PC1 + PC2 + SV_PC1 + SV_PC2 + SV_PC3 + SNV1) 
  ...
  DESeq.res <- results(dds, name = c("SNV1"), alpha = padj_limit, independentFiltering = TRUE)
