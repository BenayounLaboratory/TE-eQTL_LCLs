...
# Section 4: EBV COVARIATION PLOTS  -------------------------------------------------



# Define output dir
output.dir <- '/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/4_RNASeq_Analysis_Peptide_Exposure/3_DESeq/DESeq_Results_GM12878_rIL16/'

# Load Gene/TE expression 
my.expression <- read.csv("/Users/juanb/Desktop/2023_TE_eQTL_Juan/Code/4_RNASeq_Analysis_Peptide_Exposure/3_DESeq/Processed_counts_GM12878_rIL16/All_counts_filtered_VST.txt", header = T, row.names = 1, stringsAsFactors = F, sep = '\t')

# Define groups and expression values 
my.groups <-c(rep('T0', 3), rep('T24', 3), rep('T48', 3))

# Define expression values 
exp.EBV <- as.numeric(my.expression['EBV_gene', ])

# Make a dataframe to hold values
my.data <- data.frame(EBV = exp.EBV, groups = my.groups)

# Plot EBV vs treatment
pdf(paste(output.dir, 'EBV_Expression_vs_Treatment',".pdf", sep=""), width = 8, height = 8)
    ...
    boxplot(EBV ~ groups, 
     ...
     # overlay the points
    stripchart(EBV ~ groups, data = my.data, vertical = TRUE, add = TRUE, method = 'jitter', pch = 20, cex = 1.2, col = 'red') 
    
    # Specify axis labels
    axis(1, at = c(1, 2, 3), labels = c('T0', 'T24', 'T48'))
